#!/usr/bin/env python3
"""Fit SARIMAX on full train (≤2024), evaluate on 2025 test. Minimal memory."""
import json
import gc
import os
...
tr = combined.loc[combined.index <= train_end].copy()
te = combined.loc[combined.index > train_end].copy()

y_tr = tr[TARGET].dropna()
y_te = te[TARGET].dropna()
tr = tr.loc[y_tr.index]
te = te.loc[y_te.index]

# Truncate training to last 48000 rows to fit in 16 GB RAM
MAX_TRAIN = 48000
if len(y_tr) > MAX_TRAIN:
    print(f"Truncating train from {len(y_tr)} to {MAX_TRAIN}", flush=True)
    tr = tr.iloc[-MAX_TRAIN:]
    y_tr = y_tr.iloc[-MAX_TRAIN:]

X_tr = tr[FEAT_ARIMAX].fillna(0).values.copy()
X_te = te[FEAT_ARIMAX].fillna(0).values.copy()
y_tr_arr = y_tr.values.copy()
y_te_arr = y_te.values.copy()

del combined, tr, te, y_tr, y_te
gc.collect()

print(f"Train: {len(y_tr_arr)} | Test: {len(y_te_arr)}", flush=True)
print("Fitting SARIMAX on full train (≤2024)...", flush=True)
...
pred = mdl.predict(n_periods=len(y_te_arr), X=X_te)
r2   = r2_score(y_te_arr, pred)
rmse = mean_squared_error(y_te_arr, pred) ** 0.5
mae  = mean_absolute_error(np.expm1(y_te_arr), np.expm1(pred))

print(f"SARIMAX order: {mdl.order} x {mdl.seasonal_order}", flush=True)
print(f"Test R²={r2:.3f}  RMSE={rmse:.3f}  MAE($/MWh)={mae:.3f}", flush=True)

json.dump({
    "r2": r2, "rmse": rmse, "mae": mae,
    "order": str(mdl.order), "seasonal": str(mdl.seasonal_order)
}, open('/tmp/72_sarimax_test.json', 'w'))
